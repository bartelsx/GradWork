...
ReplayBuffer buffer;  // A shared buffer for all threads

std::atomic<int> global_episode(0); // Atomic counter to track episode numbers safely across threads
...
void run_game(Board board, Connect4Algorithm minimaxAI, DQNAgent& dqnAI, ReplayBuffer& buffer, double epsilon, std::ofstream& logFile) {
	int episode = global_episode.fetch_add(1);  // Atomically fetch the current episode and increment
	bool dqnTurn = (DQN_COLOR == Value::Red);
	bool gameOver = false;
	int moves = 0;
	std::vector<std::tuple<torch::Tensor, int, double, torch::Tensor, bool>> game_trajectory; // Collect trajectory
	...
	while (!gameOver) {
		...
		// Add experience to game trajectory
		game_trajectory.push_back({ state, move, reward, nextState, done });
		...
		if (done) {
			...
			// Lock the log file and write the episode result
			{
			...
			// Push the entire game trajectory to the ReplayBuffer after the game ends
			{
				std::lock_guard<std::mutex> lock(buffer_mutex);  // Lock the buffer to ensure thread safety
				buffer.push(game_trajectory);  // Push the entire trajectory
			}
			break;
		}
	...
	// Notify main thread to process buffer if needed (optional)
	cv.notify_all();
}
...
int main() {
	...
	const int num_threads = 32;  // Number of parallel threads to run games
	...
	{ // Training loop
		...
		// Create worker threads to run games in parallel
		for (int i = 0; i < num_threads; ++i) {
			threads.push_back(std::thread(run_game, board, minimaxAI, std::ref(dqnAI), std::ref(buffer), epsilon, std::ref(logFile)));
		}
		...
		// Process experiences collected from the threads and train the model if buffer is ready
		{
			...
			// Save model and epsilon at regular intervals
			if (episode % 200 == 0) {
				torch::save(dqnAI.policy_net, "policyReal.model");
				dqnAI.update_target();
				std::ofstream epsilon_file("epsilon.txt");
